...
class Decimal
{
    ...
    public function __construct($value, ?int $size = null, ?int $precision = null)
    {
        ...
        if (strlen($int) > ($size - $precision) || strlen($fraction) > $precision) {
            throw new ValueOutOfBoundsException($this->value, sprintf('%s(%s,%s)', self::class, $size, $precision));
        }
    ...
    public static function checkSize(int $size, int $precision): void
    {
        if ($size > self::MAX_SIZE || $size < 1 || $precision < 0) {
            throw new ValueOutOfBoundsException(sprintf('of type %s(%s,%s)', self::class, $size, $precision), self::class . '(65)');
        }
    }
    ...
    public function power($nth): self
    {
        ...
        if (!$nth->isInteger()) {
            throw new DecimalArithmeticException(sprintf(
